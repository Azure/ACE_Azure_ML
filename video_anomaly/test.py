...
# from settings import RESULTS_DIR, WEIGHTS_DIR, DATA_DIR
from scipy.ndimage import gaussian_filter


def crop(X, cropx=2, cropy=2):
    ''' Crop video frames 
    
    Arguments:
        X {numpy.ndarray} -- array of shape n_images, height, width, depth
    
    Keyword Arguments:
        cropx {int} -- how many pixels to remove left/right (default: {10})
        cropy {int} -- how many pixels to remove top/bottom (default: {10})
...
# defined dataset used for testing
dataset = 'UCSDped1' 

# settings
n_plot = 0 # How many sample sequences to plot
save_prediction_error_video_frames = True # create output that can be used to create a video
batch_size = 10 # how many epochs per batch
N_seq = None # how many videos per epoch
nt = 200 # how many frames per video sequence

# check/create path for saving output
save_path = os.path.join('outputs', dataset)
...
# print("Cropping test data and model output")
# X_test = crop(X_test)
# X_hat = crop(X_hat)
