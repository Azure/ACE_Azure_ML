...
orig_weights_file = os.path.join(WEIGHTS_DIR, 'prednet_ucsd_weights.hdf5')  # original t+1 weights
orig_json_file = os.path.join(WEIGHTS_DIR, 'prednet_ucsd_model.json')

save_model = True
extrap_weights_file = os.path.join(WEIGHTS_DIR, 'prednet_ucsd_weights-extrapfinetuned.hdf5')  # where new weights will be saved
extrap_json_file = os.path.join(WEIGHTS_DIR, 'prednet_ucsd_model-extrapfinetuned.json')

# Data files
train_file = os.path.join(KITTI_DATA_DIR, 'X_train.hkl')
train_sources = os.path.join(KITTI_DATA_DIR, 'sources_train.hkl')
val_file = os.path.join(KITTI_DATA_DIR, 'X_val.hkl')
val_sources = os.path.join(KITTI_DATA_DIR, 'sources_val.hkl')
...
callbacks.append(EarlyStopping(monitor='val_loss', min_delta=0, patience=5, verbose=0, mode='min'))
callbacks.append(CSVLogger(filename=os.path.join(RESULTS_DIR, 'train_extrap.log'), separator=',', append=False))
